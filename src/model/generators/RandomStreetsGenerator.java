...
//                    e.printStackTrace();
                    switch (key.indexOf('t')) {
                        case 0:
                            mapSouthOf(streetParts, i, j, traffic, allStreetsGrids);
                            GeneratorsUtils.initStreetPart(factories[0].newStreetPart(traffic, i, j), streetParts, allStreetsGrids, i, j);
                            break;
                        case 1:
                            mapWestOf(streetParts, i, j, traffic, allStreetsGrids);
                            GeneratorsUtils.initStreetPart(factories[1].newStreetPart(traffic, i, j), streetParts, allStreetsGrids, i, j);
                            break;
                        case 2:
                            mapNorthOf(streetParts, i, j, traffic, allStreetsGrids);
                            GeneratorsUtils.initStreetPart(factories[0].newStreetPart(traffic, i, j), streetParts, allStreetsGrids, i, j);
                            break;
                        case 3:
                            mapEastOf(streetParts, i, j, traffic, allStreetsGrids);
                            GeneratorsUtils.initStreetPart(factories[1].newStreetPart(traffic, i, j), streetParts, allStreetsGrids, i, j);
                            break;
                    }
                }
    ...
    private void mapSouthOf(StreetPart[][] streetParts, int i, int j, Traffic traffic, ObjectGrid2D allStreetGrids) {
        switch (streetParts[i + 1][j].tileIndex()) {
            case 1:
                GeneratorsUtils.initStreetPart(factories[9].newStreetPart(traffic, i + 1, j), streetParts, allStreetGrids, i + 1, j);
                break;
            case 8:
                GeneratorsUtils.initStreetPart(factories[10].newStreetPart(traffic, i + 1, j), streetParts, allStreetGrids, i + 1, j);
                break;
            case 4:
                GeneratorsUtils.initStreetPart(factories[6].newStreetPart(traffic, i + 1, j), streetParts, allStreetGrids, i + 1, j);
                break;
            case 5:
                GeneratorsUtils.initStreetPart(factories[7].newStreetPart(traffic, i + 1, j), streetParts, allStreetGrids, i + 1, j);
                break;
        }
    }
    ...
    private void mapWestOf(StreetPart[][] streetParts, int i, int j, Traffic traffic, ObjectGrid2D allStreetGrids) {
        switch (streetParts[i][j - 1].tileIndex()) {
            case 0:
                GeneratorsUtils.initStreetPart(factories[7].newStreetPart(traffic, i, j - 1), streetParts, allStreetGrids, i, j - 1);
                break;
            case 6:
                GeneratorsUtils.initStreetPart(factories[10].newStreetPart(traffic, i, j - 1), streetParts, allStreetGrids, i, j - 1);
                break;
            case 2:
                GeneratorsUtils.initStreetPart(factories[9].newStreetPart(traffic, i, j - 1), streetParts, allStreetGrids, i, j - 1);
                break;
            case 4:
                GeneratorsUtils.initStreetPart(factories[8].newStreetPart(traffic, i, j - 1), streetParts, allStreetGrids, i, j - 1);
                break;
        }
    }
    ...
    private void mapNorthOf(StreetPart[][] streetParts, int i, int j, Traffic traffic, ObjectGrid2D allStreetGrids) {
        switch (streetParts[i - 1][j].tileIndex()) {
            case 1:
                GeneratorsUtils.initStreetPart(factories[8].newStreetPart(traffic, i - 1, j), streetParts, allStreetGrids, i - 1, j);
                break;
            case 9:
                GeneratorsUtils.initStreetPart(factories[10].newStreetPart(traffic, i - 1, j), streetParts, allStreetGrids, i - 1, j);
                break;
    ...
    private void mapEastOf(StreetPart[][] streetParts, int i, int j, Traffic traffic, ObjectGrid2D allStreetGrids) {
        switch (streetParts[i][j + 1].tileIndex()) {
            case 0:
                GeneratorsUtils.initStreetPart(factories[6].newStreetPart(traffic, i, j + 1), streetParts, allStreetGrids, i, j + 1);
                break;
            case 7:
                GeneratorsUtils.initStreetPart(factories[10].newStreetPart(traffic, i, j + 1), streetParts, allStreetGrids, i, j + 1);
                break;
